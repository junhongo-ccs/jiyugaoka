...
- 補助レイヤとして `自由通り` の軸線を追加し、コーヒー店が南北に連なる読みを支える
...
- `自由通り` は注記用のラインとして薄く重ね、地元での「コーヒーストリート」的な読みを補助する

### A-4. 自由通りの注記追加

- `LineString` の手描きレイヤを 1 本追加する
- 名前は `自由通り` または `Coffee Street`
- 厳密な道路台帳線ではなく、店の並びを読むための軸線として扱う
- QGIS 側で見やすければ、後で Web 側にも同じ概念を輸出する
